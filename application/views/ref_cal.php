	    ...
	    <table class="table table-bordered fill-head subcal" >
			...
			<thead>
				...
				<tbody id="editTable">
						...
						<?php foreach($ref_costing as $key=> $sub){ ?>	
					...
					<tr>
                       ...
                       <?php }?>
							...
							?>
								...
								value="<?php //=(!empty($unit_name))?$unit_name[0]->unit_name:"";?>"></td>
								
								<?php $filter = array('unit_code'=>$sub->unit_code);
								$unit_code= $this->utilities->get_unit_name('ssr_t_uom',$filter);
								?>
								
								<td><input  type="text"  class="input-mini-xs"
								id="units_<?=$key+1?>" name="unit_code[]" 
			...
			<?php } else {?>
						...
						<td><input  required type="text"   class="input-mini-xs" id="serial_1" name="serial[]"  value=""></td>
							...
							<td><select required class="input-small" id="type_1"  placeholder="Select Category" name="item_type[]" >
						<option value="">Select Type</option>
                        <?php foreach($cost_type as $ct){?>
						 <option value="<?=$ct->costtype_desc?>"><?=$ct->costtype_desc?></option>
                       <?php }?>
						</select></td>
						<td><input type="text"  class="input-mini-xs" id="code_1" name="code[]" onblur="get_type_value(this.id,'dataTable');" value=""></td>
						<td><textarea   class="input-medium" rows="1" id="item_desc_1" name="item_desc[]"onblur="get_text_type(this.id,'dataTable');"></textarea></td>
...
function deleteRow(tableID,row, row_count)
{
			try{
				...
				adding_total('quantity_1',tableID);
           }
		catch(e)
		{
			alert(e);
