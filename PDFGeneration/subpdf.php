...
$st = $conn->prepare('SELECT * FROM ssr_t_calculation  WHERE dep_id="'.$dep_id.'" AND chap_id="'.$chap_id.'" ');
$st->execute();
$subcal = $st->fetchAll();
...
$w_item = array(20,100,30);
$w1_item = array(20,65,20,20);
$w2_item = array(20,152,20,20);
$w_class = array(120,20);
$w_si = array(30,30,20,20,20,20);
$wc_si = array(30,30,20,20,20,20);
$wc_item = array(20,20,20,22,28,40,50);


//Declare header for items
$h_item = array('S.No','Particulars of Items','Class');

//Declare header for subitem calculatio
$h_calsitem = array('Code','Unit','Quantity','Rates(in Rs.)','Amount','Total','Description');
...
foreach ($chapter as $row_chapter)
{
	...
	$pdf->MultiCell($w2_item[1],8,$row_item[5]);
	...
	{	
		...
		{	
			...
			$pdf->Cell($w1_item[0],8,$row_subitem[4],0,'L');
			$pdf->MultiCell($w1_item[1],8,$row_subitem[8],0,0,0,0,0,'L');
			
			for($i=0;$i<count($wc_item);$i++)
			{
				
			$pdf->Cell($wc_item[$i],16,$h_calsitem[$i],0,0,'L');
			}
				
			$pdf->Ln();
			
			
			foreach ($subcal as $row_subcal)
			{		
			
			
				if ($row_subcal[4]==$row_subitem[0])
				{	$y = $pdf->GetY();
					$pdf->Cell($w1_item[0],8,$row_subcal[8],0,'L');
					$pdf->Cell($w1_item[0],8,$row_subcal[10],0,0,'L');
					$pdf->Cell($w1_item[0],8,$row_subcal[11],0,0,'L');
					$pdf->Cell($w1_item[0],8,$row_subcal[12],0,0,'L');
					$pdf->Cell($w1_item[0],8,$row_subcal[13],0,0,'L');
					$pdf->Cell($w1_item[0],8,$row_subcal[14],0,0,0,0,0,'L');
					
					$pdf->MultiCell($w1_item[1],8,$row_subcal[9],0,0,0,0,0,'L');
					
					//$a2[$row_subcal[8]][$row_subcal[5]][0] = $row_subcal[9];
					//$a2[$row_subcal[8]][$row_subcal[5]][1] = $row_subcal[10];
			}
			}		
		}
	...
	foreach($a2 as $k_sid=>$v_sid)
	{
		...
		foreach($v_sid as $k_sic=>$v_sic)
		{
			...
			$pdf->SetX(120 + 20*$l);
			$pdf->set_cut_col(3+$l);
			$pdf->Cell(20,8,$v_sic[0],0,0,0,0,0,'L');
			$l = $l + 1;
